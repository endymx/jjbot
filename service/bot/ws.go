...
func ws(addr string, path string) {
	...
	logger.SugarLogger.Infof("[%s]连接CQHTTP中...", path)
	conn, _, err := websocket.DefaultDialer.Dial(u.String(), nil)
	if err != nil {
		logger.SugarLogger.Infof("[%s]连接CQHTTP失败，5秒后尝试重连...", path)
	...
	if path == "/api" {
		botapi.Api = conn
		onConnectApi()
		logger.SugarLogger.Infof("[%s]连接成功，可以开始调用api函数", path)
	} else {
		logger.SugarLogger.Infof("[%s]连接成功，正在获取消息", path)
	}
	wg.Done()
