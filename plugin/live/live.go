...
func GetLiveInfo() {
	...
	if db != nil {
		...
		data, _ := web.Post("https://api.live.bilibili.com/room/v1/Room/get_status_info_by_uids", json, true, nil)
		gjson.Get(data.String(), "data").ForEach(func(uid, user gjson.Result) bool {
...
func MassLive(db []bson.D, uid int64, message string, of bool) {
	for _, d := range db {
		...
		if uid == user {
			v12.SendGroupMsg(d.Map()["group"].(int64), message, false)
		}
	}
}
...
func AddLive(uid int, gid int64, chat bool) {
	...
	data, _ := web.Post("https://api.live.bilibili.com/room/v1/Room/get_status_info_by_uids", json, true, nil)
	var j string
	if !gjson.Get(data.String(), fmt.Sprintf("data.%d", uid)).Exists() {
		v12.SendGroupMsg(gid, "未找到用户，请检查用户id", false)
		return
	}
	if uid == 388832538 {
		j, _ = sjson.Set(data.String(), fmt.Sprintf("data.%d.uname", uid), "红米猪")
	}
	liveData := Data{
		User:     int64(uid),
		Group:    gid,
	...
	v12.SendGroupMsg(gid, "订阅主播 "+gjson.Get(j, fmt.Sprintf("data.%d.uname", uid)).String(), false)
}
...
func RemoveLive(uid int, gid int64) {
	if mongodb.Client["endymx"].FindOne("bot", "live", bson.D{{"user", uid}, {"group", gid}}, nil) != nil {
		...
		v12.SendGroupMsg(gid, "取消订阅主播 "+gjson.Get(j, fmt.Sprintf("data.%d.uname", uid)).String(), false)
	} else {
		v12.SendGroupMsg(gid, "本群未订阅过该主播", false)
	}
}
